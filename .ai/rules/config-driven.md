...
4. Add to `.env.example` in both consumer projects
...
A hardcoded operational constant only bites at the worst time: a run needs a different value, and the only way to change it is a source edit under pressure. The moment you're tempted to bump a constant "just for this run" is the moment it should have been an env var — a one-line `.env` change, not a code edit and redeploy. Wiring it to env up front is free; discovering the need mid-incident is not.
